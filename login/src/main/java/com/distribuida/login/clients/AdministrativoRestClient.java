...
public interface AdministrativoRestClient {
    ...
    @GetMapping("/docentes/{idUsuario}")
    DocenteDTO obtenerDocentePorIdUsuario(@PathVariable Integer idUsuario) ;

    @GetMapping("/docentes/existe")
    Boolean existencia(@RequestParam String cedula) ;

    @GetMapping("/admin/secredireccion/{idUsuario}")
    AdministrativoDTO obtenerAdministrativoPorIdUsuario(@PathVariable Integer idUsuario);
